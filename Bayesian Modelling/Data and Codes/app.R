...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
    sliderInput("n", "Play to ncrease sample size", min = 50,
                max = 10000, step = 10,value = 50, animate = T),
    tags$hr(),
...
post <- function(n = 50, t = 10, alpha = 1, beta = 1){
...
output$my_plot <- renderPlot({
  post(n=input$n, t = 10/50*input$n, alpha = input$alpha, beta = input$beta)
})
